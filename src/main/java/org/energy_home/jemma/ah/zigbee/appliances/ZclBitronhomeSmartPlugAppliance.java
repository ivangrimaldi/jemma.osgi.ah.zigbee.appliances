...
/**
 * 
 * @author ISMB-Pert bitronhome SMART-PLUG
 */
public class ZclBitronhomeSmartPlugAppliance extends ZclAppliance {
	private ZclEndPoint endPoint = null;

	// private static final Log log =
	// LogFactory.getLog(ZclBitronhomeSmartPlugAppliance.class); // This line is
	// related to Old Logging System
	...
	public ZclBitronhomeSmartPlugAppliance(String pid, Dictionary config) throws ApplianceException {
		...
		/**
		 * Endpoint:0x01 |-------------------------------------| | Server Side |
		 * Client Side | |-------------------------------------| | Mandatory | |
		 * Basic(0x0000) | None | | Identify(0x0003)| None | | Groups(0x0004) |
		 * None | | Scenes (0x0005) | None | | On/Off (0x0006) | None | |
		 * Optional | | Metering(0x0702)| None |
		 * |-------------------------------------|
		 */
		...
		endPoint.addServiceCluster(new ZclIdentifyServer()); // adding Identify
																// (0x0003)
		endPoint.addServiceCluster(new ZclGroupsServer()); // adding Groups
															// (0x0004)
		endPoint.addServiceCluster(new ZclScenesServer()); // adding Scenes
															// (0x0005)
		endPoint.addServiceCluster(new ZclOnOffServer()); // adding On/Off
															// (0x0006)
		endPoint.addServiceCluster(new ZclSimpleMeteringServer()); // adding
																	// Metering
																	// (0x0702)
