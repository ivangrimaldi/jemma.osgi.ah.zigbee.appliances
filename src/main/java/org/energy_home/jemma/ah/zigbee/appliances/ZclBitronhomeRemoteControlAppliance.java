...
/**
 * 
 * @author ISMB-Pert bitronhome Remote Control with four buttons
 */
public class ZclBitronhomeRemoteControlAppliance extends ZclAppliance {
	private ZclEndPoint endPoint = null;

	// private static final Log log =
	// LogFactory.getLog(ZclBitronhomeRemoteControlAppliance.class); // This
	// line is related to Old Logging System
	...
	public ZclBitronhomeRemoteControlAppliance(String pid, Dictionary config) throws ApplianceException {
		 ...
		 * |-------------------------------------------------| | Mandatory | |
		 * Basic(0x0000) | Basic(0x0000) | | Identify(0x0003) | On/Off(0x0006) |
		 * | None |Level Control(0x0008)| | Optional | |Power
		 * Configuration(0x0001)| None |
		 * |-------------------------------------------------|
		 */

		// Server Clusters
		endPoint.addServiceCluster(new ZclBasicServer()); // adding Basic
															// (0x0000)
		endPoint.addServiceCluster(new ZclIdentifyServer()); // adding Identify
																// (0x0003)
		endPoint.addServiceCluster(new ZclPowerConfigurationServer()); // adding
																		// Power
																		// Configuration(0x0001)
		// Client Clusters
		endPoint.addServiceCluster(new ZclBasicClient()); // adding Basic
															// (0x0000)
		endPoint.addServiceCluster(new ZclOnOffClient()); // adding On/Off
															// (0x0006)
		endPoint.addServiceCluster(new ZclLevelControlClient()); // adding Level
																	// Control
																	// (0x0008)
