...
public class ZclColorLightApplianceFactory extends DriverApplianceFactory implements Driver {
	public static final String APPLIANCE_TYPE = "org.energy_home.jemma.ah.zigbee.colorlight";
	...
	public Appliance getInstance(String pid, Dictionary config) throws ApplianceException {
		return new ZclColorLightAppliance(pid, config);
	}

	public String deviceMatchFilterString() {
		return "(&(DEVICE_CATEGORY=ZigBee)(zigbee.device.profile.id=49246)(|(zigbee.device.device.id=512)(zigbee.device.device.id=528)))";
	}
}
