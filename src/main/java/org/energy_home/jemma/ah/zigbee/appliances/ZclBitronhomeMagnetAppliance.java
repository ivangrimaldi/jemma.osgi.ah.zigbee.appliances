...
public class ZclBitronhomeMagnetAppliance extends ZclAppliance {

	private ZclEndPoint endPoint = null;

	private static final Logger LOG = LoggerFactory.getLogger(ZclBitronhomeMagnetAppliance.class);

	public ZclBitronhomeMagnetAppliance(String pid, Dictionary config) throws ApplianceException {
		super(pid, config);
		endPoint = this.zclAddEndPoint(IEndPointTypes.ZIGBEE_LEVEL_CONTROLLABLE_OUTPUT);

		// Bitronhome Magnet sensor
		/**
		 * Endpoint:0x01 |-------------------------------------------------| |
		...
		endPoint.addServiceCluster(new ZclIdentifyServer()); // adding Identify
																// (0x0003)
		endPoint.addServiceCluster(new ZclIASZoneServer()); // adding IAS
															// Zone(0x0500)
